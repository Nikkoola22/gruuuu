      ...
      <div className="space-y-5">
        ...
        {horaireBaseType === 'indice' ? (
          <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 shadow-sm">
            ...
            />
            {horaireTI > 0 && (
              <div className="mt-3 text-sm text-purple-700 font-medium space-y-1">
  ...
  const renderStepResultat = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Total principal */}
        <div className={`bg-${accentColor}-50 rounded-2xl p-6 border border-${accentColor}-200 shadow-sm`}>
          <p className={`text-xs uppercase tracking-wider text-${accentColor}-800 font-bold mb-2`}>Total estimé</p>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col">
      ...
      <div className="bg-white dark:bg-slate-800/95 dark:bg-slate-900/95 backdrop-blur-md py-6 border-b border-slate-200 shadow-sm glass-banner">
        <div className="px-4 sm:px-6 flex flex-col gap-4 max-w-6xl mx-auto sm:flex-row sm:items-center sm:justify-between">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="flex w-full items-center justify-center gap-2 px-5 py-2.5 bg-red-50 hover:bg-red-100 text-red-700 rounded-lg font-medium transition-all duration-150 border border-red-200 shadow-sm glass-pill sm:w-auto"
            >
                ...
                <div key={step.id} className="flex items-center">
                  ...
                  {index < STEPS.length - 1 && (
                    <div className={`w-8 sm:w-16 h-0.5 mx-1 sm:mx-2 transition-all duration-200 ${
                      currentStep > step.id ? `bg-${accentColor}-400` : 'bg-slate-200'
                    }`} />
                  )}
                </div>
              )
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        <div className="max-w-2xl mx-auto">
          {/* Tip de l'étape */}
          {currentStep < 5 && (
            <div className={`mb-6 p-4 rounded-xl border glass-card shadow-sm ${
              agentType === 'horaire' 
                ? 'bg-purple-50 border-purple-200' 
                : 'bg-emerald-50 border-emerald-200'
            }`}>
              <p className={`text-sm font-medium flex items-start gap-2 ${
                agentType === 'horaire' ? 'text-purple-800' : 'text-emerald-800'
              }`}>
                <Info className="w-4 h-4 mt-0.5 flex-shrink-0" />
          ...
          {currentStep < 5 && (
            <div className="flex flex-col-reverse items-stretch mt-6 gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                onClick={goNext}
                disabled={!canGoNext()}
                className={`flex w-full items-center justify-center gap-2 px-6 py-2.5 rounded-lg font-medium transition-all duration-150 sm:w-auto shadow-sm ${
                  canGoNext()
                    ? agentType === 'horaire'
