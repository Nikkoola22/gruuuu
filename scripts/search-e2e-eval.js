...
const NOT_FOUND_PATTERNS = [
  'je ne trouve pas cette information dans nos documents internes',
  'pas trouve cette information dans nos documents internes',
  'aucune information dans nos documents internes',
  'ne trouve pas cette information dans nos documents',
];
...
let cacheTemps = null;
let cacheFormation = null;
let cacheTeletravail = null;
let cacheRifseep = null;

function loadActualContent(entry) {
  if (!cacheTemps) {
    try {
      const filePath = path.join(repoRoot, 'src/data/temps.ts');
      const content = fs.readFileSync(filePath, 'utf-8');
      cacheTemps = {};
      for (let i = 1; i <= 4; i++) {
        const regex = new RegExp(`\\b${i}:\\s*\`([\\s\\S]*?)\`\\.trim\\(\\)`, 'm');
        const match = regex.exec(content);
        if (match) {
          cacheTemps[i] = match[1].trim();
        } else {
          const regexFallback = new RegExp(`\\b${i}:\\s*\`([\\s\\S]*?)\``, 'm');
          const matchFallback = regexFallback.exec(content);
          if (matchFallback) {
            cacheTemps[i] = matchFallback[1].trim();
          }
        }
      }
    } catch (err) {
      console.error('Error loading temps.ts:', err);
    }
  }

  if (!cacheFormation) {
    try {
      const filePath = path.join(repoRoot, 'src/data/formation.ts');
      const content = fs.readFileSync(filePath, 'utf-8');
      const match = /export\s+const\s+formation\s*=\s*`([\s\S]*?)`/.exec(content);
      cacheFormation = match ? match[1].trim() : '';
    } catch (err) {
      console.error('Error loading formation.ts:', err);
    }
  }

  if (!cacheTeletravail) {
    try {
      const filePath = path.join(repoRoot, 'src/data/teletravail.ts');
      const content = fs.readFileSync(filePath, 'utf-8');
      const match = /export\s+const\s+teletravailData\s*=\s*`([\s\S]*?)`/.exec(content);
      cacheTeletravail = match ? match[1].trim() : '';
    } catch (err) {
      console.error('Error loading teletravail.ts:', err);
    }
  }

  if (!cacheRifseep) {
    try {
      const filePath = path.join(repoRoot, 'src/data/rifseep-data.ts');
      const content = fs.readFileSync(filePath, 'utf-8');
      const match = /export\s+const\s+rifseepData\s*=\s*`([\s\S]*?)`/.exec(content);
      cacheRifseep = match ? match[1].trim() : '';
    } catch (err) {
      console.error('Error loading rifseep-data.ts:', err);
    }
  }

  if (entry.source === 'temps' && entry.chapitre) {
    return cacheTemps && cacheTemps[entry.chapitre] ? cacheTemps[entry.chapitre] : '';
  }
  if (entry.source === 'formation') {
    return cacheFormation || '';
  }
  if (entry.source === 'teletravail') {
    return cacheTeletravail || '';
  }
  if (entry.source === 'rifseep') {
    return cacheRifseep || '';
  }
  return '';
}
...
function buildContext(topSommaire, topBip) {
  const sommairePart = topSommaire
    .map((entry, idx) => {
      const actualContent = loadActualContent(entry);
      const contentToUse = actualContent || `Résumé: ${entry.resume || 'N/A'}`;
      return `[Sommaire ${idx + 1}] ${entry.id} - ${entry.titre}\n${contentToUse}`;
    })
    .join('\n\n');
